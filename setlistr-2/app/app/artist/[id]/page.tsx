...
export default function ArtistProfilePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Stats — venue name must contain a space to filter out single-word test data
      // e.g. "Mitchell Delicatessen" passes, "Jesse" does not
      const venueCounts: Record<string, number> = {}
      const cityCounts: Record<string, number>  = {}
      perfs.forEach(p => {
        const venue = p.venue_name?.trim()
        if (venue && venue.length >= 4 && venue.includes(' ')) {
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 600, margin: '0 auto', padding: '0 16px', boxSizing: 'border-box' }}>
        ...
        <div style={{ paddingTop: 32, paddingBottom: 24, animation: 'fadeUp 0.4s ease' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: `radial-gradient(circle at 35% 35%, ${C.gold}40, ${C.gold}15)`, border: `1px solid ${C.gold}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span style={{ fontSize: 24, fontWeight: 800, color: C.gold }}>{artistName.charAt(0).toUpperCase()}</span>
            </div>
            <div>
              <h1 style={{ fontSize: 28, fontWeight: 800, color: C.text, margin: 0, letterSpacing: '-0.03em' }}>{artistName}</h1>
          ...
          <div style={{ background: C.goldDim, border: `1px solid ${C.borderGold}`, borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: C.gold, margin: '0 0 4px' }}>Estimated Royalties</p>
        ...
        {/* Top venue / city — only renders if valid data exists */}
        {(stats.topVenue || stats.topCity) && (
          <div style={{ display: 'grid', gridTemplateColumns: stats.topVenue && stats.topCity ? '1fr 1fr' : '1fr', gap: 8, marginBottom: 24, animation: 'fadeUp 0.4s 0.08s ease both' }}>
            {stats.topVenue && (
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 4, animation: 'fadeUp 0.4s 0.1s ease both' }}>
          {(['overview', 'shows', 'songs'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              style={{ flex: 1, padding: '9px', border: 'none', borderRadius: 9, background: activeTab === tab ? C.goldDim : 'transparent', color: activeTab === tab ? C.gold : C.muted, fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'capitalize', cursor: 'pointer', transition: 'all 0.15s ease', fontFamily: 'inherit' }}>
              {tab}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'overview' && (
          <div style={{ animation: 'fadeUp 0.3s ease' }}>
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: C.muted, margin: '0 0 10px' }}>Most Played Songs</p>
            {topSongs.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 0', color: C.muted, fontSize: 13 }}>No songs recorded yet</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {topSongs.map((song, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.card, border: `1px solid ${i === 0 ? C.borderGold : C.border}`, borderRadius: 11, padding: '11px 14px', animation: `fadeUp 0.3s ${i * 0.04}s ease both` }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: i === 0 ? C.gold : C.muted, minWidth: 20, textAlign: 'right', fontFamily: '"DM Mono", monospace', fontVariantNumeric: 'tabular-nums' }}>{i + 1}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 14, fontWeight: 600, color: i === 0 ? C.text : C.secondary, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{song.title}</p>
                      {song.artist && <p style={{ fontSize: 11, color: C.muted, margin: '2px 0 0' }}>{song.artist}</p>}
                    </div>
                    <div style={{ background: i === 0 ? C.goldDim : 'rgba(255,255,255,0.04)', border: `1px solid ${i === 0 ? C.borderGold : C.border}`, borderRadius: 20, padding: '3px 10px', flexShrink: 0 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: i === 0 ? C.gold : C.muted, fontFamily: '"DM Mono", monospace' }}>×{song.count}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'shows' && (
          <div style={{ animation: 'fadeUp 0.3s ease' }}>
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: C.muted, margin: '0 0 10px' }}>All Shows</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {performances.map((perf, i) => (
                <div key={perf.id} onClick={() => router.push(`/app/review/${perf.id}`)}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.card, border: `1px solid ${C.border}`, borderRadius: 11, padding: '12px 14px', cursor: 'pointer', transition: 'border-color 0.15s, background 0.15s', animation: `fadeUp 0.3s ${i * 0.03}s ease both` }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 14, fontWeight: 600, color: C.text, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{perf.venue_name || 'Unknown Venue'}</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3 }}>
                      {perf.city && <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 11, color: C.muted }}><MapPin size={9} />{perf.city}</span>}
                      {perf.started_at && <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 11, color: C.muted }}><Clock size={9} />{formatDate(perf.started_at)}</span>}
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: statusColor(perf.status) }} />
                    <span style={{ fontSize: 10, color: statusColor(perf.status), fontWeight: 600, letterSpacing: '0.06em' }}>{statusLabel(perf.status)}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'songs' && (
          <div style={{ animation: 'fadeUp 0.3s ease' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {topSongs.map((song, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: '10px 14px', animation: `fadeUp 0.3s ${i * 0.02}s ease both` }}>
                  ...
                  <span style={{ fontSize: 10, color: C.muted, fontFamily: '"DM Mono", monospace', flexShrink: 0 }}>{song.count}× played</span>
                </div>
              ))}
            </div>
          </div>
