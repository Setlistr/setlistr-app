...
export default function AdminDashboard({
  ...
  const artists = useMemo(() => {
    ...
    return Array.from(profileMap.values()).map(profile => {
      ...
      return { id: profile.id, name: displayName, hasName, pro: profile.pro_affiliation || '—', showCount: userPerfs.length, songCount: userSongList.length, submitted, lastPerf }
    })
  ...
  const topSongs = useMemo(() => {
    ...
    performanceSongs.forEach(s => {
      ...
      if (existing) { existing.count++; if (s.isrc) existing.isrcCount++ }
      else map.set(key, { title: s.title, artist: s.artist, count: 1, isrcCount: s.isrc ? 1 : 0 })
    })
  ...
  const venues = useMemo(() => {
    ...
    performances.forEach(p => {
      ...
      const existing = map.get(key)
      if (existing) { existing.showCount++; if (p.started_at > existing.lastShow) existing.lastShow = p.started_at }
      else map.set(key, { name: p.venue_name, city: p.city || '', showCount: 1, lastShow: p.started_at })
    })
  ...
  async function deletePerformance(id: string, venueName: string) {
    ...
    try {
      ...
      if (res.ok) setLocalPerfs(prev => prev.filter(p => p.id !== id))
  ...
  async function deleteShow(perfId: string) {
    ...
    try {
      ...
      if (res.ok) setLocalPerfs(prev => prev.filter(p => p.id !== perfId))
      else { const err = await res.json().catch(() => ({})); alert('Failed to delete: ' + (err.error || res.status)) }
  ...
  async function forceEndShow(perfId: string) {
    ...
    try {
      const res = await fetch('/api/admin/delete-show', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ performance_id: perfId }) })
  ...
  function exportCSV(type: 'shows' | 'songs' | 'detection') {
    ...
    if (type === 'shows') {
      filename = 'setlistr-shows.csv'; csv = 'Date,Venue,City,Artist,Status,Submitted,Songs\n'
      performances.forEach(p => { const songCount = performanceSongs.filter(s => s.performance_id === p.id).length; csv += `"${new Date(p.started_at).toLocaleDateString()}","${p.venue_name || ''}","${p.city || ''}","${p.artist_name || ''}","${p.status}","${p.submission_status || 'no'}","${songCount}"\n` })
    } else if (type === 'songs') {
      filename = 'setlistr-songs.csv'; csv = 'Title,Artist,ISRC,Composer,Performance ID\n'
      performanceSongs.forEach(s => { csv += `"${s.title}","${s.artist}","${s.isrc || ''}","${s.composer || ''}","${s.performance_id}"\n` })
    } else {
      filename = 'setlistr-detection.csv'; csv = 'Date,Title,Artist,Score,Confidence,Venue,Source\n'
      detectionEvents.forEach(e => { csv += `"${new Date(e.created_at).toLocaleString()}","${e.final_title || e.acr_title || ''}","${e.acr_artist || ''}","${e.acr_score}","${e.confidence_level || ''}","${e.venue_name || ''}","${e.final_source || ''}"\n` })
    }
  ...
  async function importSpotifyForArtist(userId: string, artistName: string) {
    ...
    try {
      const res = await fetch('/api/admin/spotify-import', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, artist_name: searchName }) })
  ...
  async function assignDelegate() {
    ...
    try {
      const res = await fetch('/api/admin/assign-delegate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ artist_id: saArtistId.trim(), delegate_id: saManagerId.trim() }) })
      const data = await res.json()
  ...
  async function preloadSetlist() {
    ...
    try {
      const res = await fetch('/api/admin/preload-setlist', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: saSetlistArtistId.trim(), songs }) })
      const data = await res.json()
        ...
        {tab === 'overview' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: '18px 20px' }}>
              ...
              <p style={{ fontSize: 11, color: C.muted, margin: '14px 0 0' }}>{quality.total} total songs across all performances</p>
            </div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: '18px 20px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {(() => {
                  ...
                  profiles.forEach(p => { const pro = p.pro_affiliation || 'None'; proCounts[pro] = (proCounts[pro] || 0) + 1 })
                  return Object.entries(proCounts).sort((a, b) => b[1] - a[1]).map(([pro, count]) => (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {([
                ...
                <button key={f.id} onClick={() => setDetFilter(f.id)}
                  style={{ padding: '6px 14px', borderRadius: 20, fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit', border: `1px solid ${detFilter === f.id ? C.gold : C.border}`, background: detFilter === f.id ? C.goldDim : 'transparent', color: detFilter === f.id ? C.gold : C.muted, transition: 'all 0.15s ease' }}>
                  {f.label}
                </button>
              ))}
            </div>
                ...
                return (
                  <div key={e.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, overflow: 'hidden' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px' }}>
                      <div style={{ marginTop: 3, flexShrink: 0 }}><Dot color={isDetected ? C.green : C.red} /></div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        {isDetected ? (
                          <>
                            <p style={{ fontSize: 14, fontWeight: 600, color: C.text, margin: 0 }}>{e.final_title || e.acr_title}</p>
                      ...
                      <button onClick={() => setExpandedId(isExpanded ? null : e.id)} style={{ background: 'none', border: 'none', color: C.muted, cursor: 'pointer', padding: 4, flexShrink: 0, fontSize: 14 }}>
                        {isExpanded ? '▲' : '▼'}
                      </button>
                    </div>
                    {isExpanded && (
              ...
              <div key={a.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: 11, color: C.muted, margin: '2px 0 10px' }}>{a.pro !== '—' ? a.pro : 'No PRO set'} · Last show: {a.lastPerf ? timeAgo(a.lastPerf) : 'never'}</p>
                    <div style={{ display: 'flex', gap: 16 }}>
                      {[{ label: 'Shows', value: a.showCount }, { label: 'Songs', value: a.songCount }, { label: 'Submitted', value: a.submitted }].map(s => (
                        <div key={s.label}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
                    {a.submitted > 0 && <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: C.green, background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)', borderRadius: 6, padding: '3px 8px' }}>Submitted</span>}
                    {(() => {
                      const state = spotifyImporting[a.id] || 'idle'
                      return (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          <input value={spotifyOverride[a.id] ?? ''} onChange={e => setSpotifyOverride(prev => ({ ...prev, [a.id]: e.target.value }))} placeholder={a.name} title="Override artist name for Spotify search"
                          ...
                          <button onClick={() => importSpotifyForArtist(a.id, a.name)} disabled={state === 'loading'}
        ...
        {tab === 'songs' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <input value={songSearch} onChange={e => setSongSearch(e.target.value)} placeholder="Search songs..."
              style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: '11px 14px', color: C.text, fontSize: 14, outline: 'none', fontFamily: 'inherit', width: '100%', boxSizing: 'border-box' as const }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {filteredSongs.map((s, i) => (
                <div key={s.title + s.artist} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: C.card, border: `1px solid ${C.border}`, borderRadius: 10 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                    {s.isrcCount > 0 && <span style={{ fontSize: 9, color: C.green, fontFamily: '"DM Mono", monospace', opacity: 0.7 }}>ISRC</span>}
        ...
        {tab === 'superadmin' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: '18px 20px' }}>
              ...
              <p style={{ fontSize: 11, color: C.muted, margin: '0 0 14px' }}>Pick the artist being managed, then pick who gets access.</p>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: '18px 20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div>
                  ...
                  <textarea value={saSetlistSongs} onChange={e => setSaSetlistSongs(e.target.value)} placeholder={'Five Beers From Now\nNobody Wins\nPure Excuses'} rows={8}
                    style={{ width: '100%', background: '#0f0e0c', border: `1px solid ${C.border}`, borderRadius: 8, padding: '10px 12px', color: C.text, fontSize: 13, fontFamily: 'inherit', outline: 'none', resize: 'vertical', lineHeight: 1.8, boxSizing: 'border-box' as const }} />
                </div>
