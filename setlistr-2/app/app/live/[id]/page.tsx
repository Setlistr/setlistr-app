...
export default function LiveCapturePage({ params }: { params: { id: string } }) {
    ...
    supabase.from('performances_visible').select('*, show_id, setlist_id, artist_id').eq('id', params.id).single()
      .then(({ data }) => {
        if (data) {
          ...
          supabase2.from('performance_songs_visible')
  ...
  return (
    <div style={{ minHeight: '100svh', background: C.bg, display: 'flex', flexDirection: 'column', fontFamily: '"DM Sans", system-ui, sans-serif', overflowX: 'clip', overscrollBehavior: 'none', paddingTop: 'env(safe-area-inset-top)' }}>
